...
## 13. 実装タスク（カテゴリルールCSV インポート/エクスポート）
- [ ] CSVフォーマットを `mode,large,middle`（UTF-8, ヘッダ必須）で実装。`mode` は `whitelist` / `blacklist`。
- [ ] options 画面に「ルールをエクスポート」「ルールをインポート」のUIを追加（input type="file" accept=".csv" を含む）。
- [ ] エクスポート: 現在の whitelist/blacklist を mode 列付きで1ファイルに書き出し、ダウンロードさせる。
- [ ] インポート: CSVをパースし、正規化後に既存キーは「上書き」、新規は追加。上限超過時はエラー。
- [ ] パースエラー／列欠如／件数超過／空ファイルのエラーメッセージを表示。成功時に追加/上書き件数をステータス表示。
- [ ] 入力サイズ上限（例: 200KB）を設定し、大容量ファイルは拒否。
- [ ] テスト: 正常系（BOMあり/なし、混在順）、重複上書き、上限超過、欠損列、空ファイル。
